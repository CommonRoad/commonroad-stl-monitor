...
class VelocityPredicateCollection(PredicateCollection):
    ...
    def _speed_limit_suggested(self, time_step: int, vehicle: Vehicle) -> float:
        ...
        :returns speed limit
        """
        v_max_lane = self._traffic_sign_interpreter.speed_limit(
            frozenset(vehicle.lanelet_assignment[time_step])
        )
        if v_max_lane is None or v_max_lane == float("inf"):
            return self._traffic_rules_param.get("desired_interstate_velocity")
        else:
            return min(
                self._traffic_rules_param.get("desired_interstate_velocity"), v_max_lane
            )

    def slow_leading_vehicle(
        self,
        time_step: int,
        vehicle: Vehicle,
        ...
        for veh_o in other_vehicles:
            if veh_o.states_lon.get(time_step) is None:
                continue
            if not PositionPredicateCollection.in_front_of(
                time_step, vehicle, veh_o, OperatingMode.MONITOR
            ) or not PositionPredicateCollection.in_same_lane_classmethod(
                self._road_network.find_lane_ids_by_lanelets(lanelets_veh),
                self._road_network.find_lane_ids_by_lanelets(
                    veh_o.lanelet_assignment[time_step]
                ),
            ):
                continue
            ...
            if v_max - veh_o.states_lon[time_step].v >= self._traffic_rules_param.get(
                "min_velocity_dif"
            ):
    ...
    def preserves_traffic_flow(
        ...
        if operating_mode is OperatingMode.MONITOR:
            if v_max - vehicle.states_lon[time_step].v < self._traffic_rules_param.get(
                "min_velocity_dif"
            ):
                return True
            else:
            ...
            return (
                vehicle.states_lon[time_step].v
                - v_max
                + self._traffic_rules_param.get("min_velocity_dif")
                - 1e-17
            )
    ...
    def in_standstill(
        ...
        if operating_mode is OperatingMode.MONITOR:
            ...
            return abs(self._traffic_rules_param.get("standstill_error")) - abs(
                vehicle.states_lon[time_step].v
            )

    def exist_standing_leading_vehicle(
        self,
        time_step: int,
        vehicle: Vehicle,
        other_vehicles: List[Vehicle],
    ...
    def drives_with_slightly_higher_speed(
        self,
        time_step: int,
        vehicle_k: Vehicle,
        vehicle_p: Vehicle,
        ...
        if operating_mode is OperatingMode.MONITOR:
            if (
                0
                < vehicle_k.states_lon[time_step].v - vehicle_p.states_lon[time_step].v
    ...
    def reverses(
        ...
        """
        if operating_mode is OperatingMode.MONITOR:
            if vehicle.states_lon[time_step].v < -self._traffic_rules_param.get(
                "standstill_error"
            ):
                return True
            else:
    ...
    def keeps_sign_min_speed_limit(
        self, time_step: int, vehicle: Vehicle, operating_mode: OperatingMode
    ) -> Union[bool, float, Constraint]:
        """
        ...
        required_speed = self._traffic_sign_interpreter.required_speed(
        ...
        if operating_mode is OperatingMode.MONITOR:
            ...
            if required_speed is None:
                return Constraint(
                    [ConstraintType.VELOCITY], ConstraintRepresentation.LOWER, 0
                )
            else:
                return Constraint(
                    [ConstraintType.VELOCITY],
    ...
    @staticmethod
    def keeps_fov_speed_limit(
        time_step: int, vehicle: Vehicle, operating_mode: OperatingMode
    ) -> Union[bool, float, Constraint]:
        """
    ...
    def keeps_type_speed_limit(
        ...
        """
        if operating_mode is OperatingMode.MONITOR:
            if vehicle.states_lon[time_step].v <= self._get_type_speed_limit(
    ...
    def keeps_lane_speed_limit(
        ...
        if operating_mode is OperatingMode.MONITOR:
            ...
            if speed_limit is None:
                return Constraint(
                    [ConstraintType.VELOCITY], ConstraintRepresentation.UPPER, math.inf
                )
            else:
                return Constraint(
                    [ConstraintType.VELOCITY],
    ...
    def evaluate_predicates_online(
        ...
        time_step: int,
        operating_mode: OperatingMode,
    ) -> Dict[str, Dict[int, Union[bool, float]]]:
        # [(var: float), (var: float)]
        # {var: {vehicle_id: float}}
        predicate_current_time_step = {
            "keeps_lane_speed_limit__x_ego": {},
            "keeps_fov_speed_limit__x_ego": {},
            "preserves_traffic_flow__x_ego": {},
            "slow_leading_vehicle__x_ego": {},
            "keeps_sign_min_speed_limit__x_ego": {},
            "keeps_braking_speed_limit__x_ego": {},
            "keeps_type_speed_limit__x_ego": {},
            "exist_standing_leading_vehicle__x_ego": {},
            "in_standstill__x_ego": {},
        ...
        if "keeps_lane_speed_limit__x_ego" in self._necessary_predicates:
            predicate_current_time_step["keeps_lane_speed_limit__x_ego"][
                ego_vehicle.id
        ...
        if "keeps_fov_speed_limit__x_ego" in self._necessary_predicates:
            predicate_current_time_step["keeps_fov_speed_limit__x_ego"][
                ego_vehicle.id
        ...
        if "keeps_braking_speed_limit__x_ego" in self._necessary_predicates:
            predicate_current_time_step["keeps_braking_speed_limit__x_ego"][
                ego_vehicle.id
        ...
        if "preserves_traffic_flow__x_ego" in self._necessary_predicates:
            predicate_current_time_step["preserves_traffic_flow__x_ego"][
                ego_vehicle.id
        ...
        if "slow_leading_vehicle__x_ego" in self._necessary_predicates:
            predicate_current_time_step["slow_leading_vehicle__x_ego"][
                ego_vehicle.id
            ] = self.slow_leading_vehicle(
                time_step, ego_vehicle, other_vehicles, operating_mode
            )
        if "keeps_type_speed_limit__x_ego" in self._necessary_predicates:
            predicate_current_time_step["keeps_type_speed_limit__x_ego"][
                ego_vehicle.id
        ...
        if "keeps_sign_min_speed_limit__x_ego" in self._necessary_predicates:
            predicate_current_time_step["keeps_sign_min_speed_limit__x_ego"][
                ego_vehicle.id
        ...
        if "exist_standing_leading_vehicle__x_ego" in self._necessary_predicates:
            predicate_current_time_step["exist_standing_leading_vehicle__x_ego"][
                ego_vehicle.id
            ] = self.exist_standing_leading_vehicle(
                time_step, ego_vehicle, other_vehicles, operating_mode
            )
        if "in_standstill__x_ego" in self._necessary_predicates:
            predicate_current_time_step["in_standstill__x_ego"][
                ego_vehicle.id
            ] = self.in_standstill(time_step, ego_vehicle, operating_mode)
        if "reverses__x_ego" in self._necessary_predicates:
            predicate_current_time_step["reverses__x_ego"][
                ego_vehicle.id
        ...
        for other_vehicle in other_vehicles:
            ...
            if (
                "drives_with_slightly_higher_speed__x_ego__x_o"
                in self._necessary_predicates
            ):
                predicate_current_time_step[
                    "drives_with_slightly_higher_speed__x_ego__x_o"
                ][other_vehicle.id] = self.drives_with_slightly_higher_speed(
                    time_step, ego_vehicle, other_vehicle, operating_mode
                )
    ...
    def evaluate_predicates(
        ...
        """
        predicate_trace = {
            "keeps_lane_speed_limit__x_ego": {ego_vehicle.id: {}},
            # {var: {vehicle_id: {time_step: {float}}}}
        ...
        for time_step in ego_vehicle.states_lon.keys():
            ...
            if "keeps_fov_speed_limit__x_ego" in self._necessary_predicates:
                predicate_trace["keeps_fov_speed_limit__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.keeps_fov_speed_limit(time_step, ego_vehicle, operating_mode)
            if "keeps_braking_speed_limit__x_ego" in self._necessary_predicates:
                predicate_trace["keeps_braking_speed_limit__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.keeps_braking_speed_limit(
                    time_step, ego_vehicle, operating_mode
                )
            if "preserves_traffic_flow__x_ego" in self._necessary_predicates:
                predicate_trace["preserves_traffic_flow__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.preserves_traffic_flow(time_step, ego_vehicle, operating_mode)
            if "slow_leading_vehicle__x_ego" in self._necessary_predicates:
                predicate_trace["slow_leading_vehicle__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.slow_leading_vehicle(
                    time_step, ego_vehicle, other_vehicles, operating_mode
                )
            if "keeps_type_speed_limit__x_ego" in self._necessary_predicates:
                predicate_trace["keeps_type_speed_limit__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.keeps_type_speed_limit(time_step, ego_vehicle, operating_mode)
            if "keeps_sign_min_speed_limit__x_ego" in self._necessary_predicates:
                predicate_trace["keeps_sign_min_speed_limit__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.keeps_sign_min_speed_limit(
                    time_step, ego_vehicle, operating_mode
                )
            if "exist_standing_leading_vehicle__x_ego" in self._necessary_predicates:
                predicate_trace["exist_standing_leading_vehicle__x_ego"][
                    ego_vehicle.id
                ][time_step] = self.exist_standing_leading_vehicle(
                    time_step, ego_vehicle, other_vehicles, operating_mode
                )
            if "in_standstill__x_ego" in self._necessary_predicates:
                predicate_trace["in_standstill__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.in_standstill(time_step, ego_vehicle, operating_mode)
            if "reverses__x_ego" in self._necessary_predicates:
                predicate_trace["reverses__x_ego"][ego_vehicle.id][
                    time_step
                ] = self.reverses(time_step, ego_vehicle, operating_mode)
        ...
        for other_vehicle in other_vehicles:
            ...
            predicate_trace["drives_with_slightly_higher_speed__x_ego__x_o"][
                other_vehicle.id
            ] = {}
            for time_step in ego_vehicle.states_lon.keys():
                if other_vehicle.states_lon.get(time_step) is None:
                ...
                if (
                    ...
                    predicate_trace["drives_with_slightly_higher_speed__x_ego__x_o"][
                        other_vehicle.id
                    ][time_step] = self.drives_with_slightly_higher_speed(
                        time_step, ego_vehicle, other_vehicle, operating_mode
                    )
