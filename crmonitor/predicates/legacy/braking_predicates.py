...
class BrakingPredicateCollection(PredicateCollection):
    ...
    def unnecessary_braking(
        self,
        time_step: int,
        ego_vehicle: Vehicle,
        other_vehicles: List[Vehicle],
        operating_mode: OperatingMode,
    ) -> Union[bool, Constraint, float]:
        ...
        for veh_o in other_vehicles:
            ...
            if (
                PositionPredicateCollection.in_front_of(
                    time_step, ego_vehicle, veh_o, OperatingMode.MONITOR
                )
                ...
                and self.keeps_safe_distance_prec(
                    time_step, ego_vehicle, veh_o, OperatingMode.MONITOR
                )
            ):
        ...
        if operating_mode is operating_mode.MONITOR:
            if (
                same_lane_front_vehicle is False
                and a_ego < self._traffic_rules_param.get("a_abrupt")
            ):
                # no leading vehicle
                return True
            else:
            ...
            if same_lane_front_vehicle is False:
                return (
                    self._traffic_rules_param.get("a_abrupt") - a_ego
                )  # a_ego < a_abrupt
            else:
    ...
    def safe_distance(
        ...
        """
        assert (
            a_min_follow and 0 > a_min_lead
        ), "<BrakingPredicateCollection/safe_distance>: acceleration is not valid"
        d_safe = (
            (v_lead ** 2) / (-2 * abs(a_min_lead))
            - (v_follow ** 2) / (-2 * abs(a_min_follow))
    ...
    @staticmethod
    def keeps_safe_distance_prec(
        time_step: int,
        vehicle_follow: Vehicle,
        ...
        safe_distance = BrakingPredicateCollection.safe_distance(
    ...
    def brakes_stronger(
        ...
        if operating_mode is OperatingMode.MONITOR:
            if (
                vehicle_k.states_lon[time_step].a < vehicle_p.states_lon[time_step].a
        ...
        ):  # returns upper bound for acceleration
            return Constraint(
                [ConstraintType.ACCELERATION],
                ConstraintRepresentation.LOWER,
            ...
            operating_mode is OperatingMode.ROBUSTNESS
        ):  # returns difference to upper bound defined by constraint
            return (
                min(vehicle_p.states_lon[time_step].a, 0)
                - vehicle_k.states_lon[time_step].a
            )
        ...
        for other_vehicle in other_vehicles:
            if other_vehicle.states_lon.get(time_step) is None:
                if operating_mode == OperatingMode.ROBUSTNESS:
                    ...
                    value = True
                predicate_current_time_step["keeps_safe_distance_prec__x_ego__x_o"][
                    other_vehicle.id
                ] = value
                predicate_current_time_step["keeps_safe_distance_prec__x_o__x_ego"][
                    other_vehicle.id
                ] = value
                predicate_current_time_step["brakes_stronger__x_ego__x_o"][
                    other_vehicle.id
                ] = value
                continue
    ...
    def evaluate_predicates(
        ...
        """
        predicate_trace = {
            "unnecessary_braking__x_ego": {ego_vehicle.id: {}},
            "keeps_safe_distance_prec__x_ego__x_o": {},
            "keeps_safe_distance_prec__x_o__x_ego": {},
            "brakes_stronger__x_ego__x_o": {},
        }
        ...
        for other_vehicle in other_vehicles:
            ...
            for time_step in range(time_interval[0], time_interval[1] + 1):
                ...
                if "keeps_safe_distance_prec__x_ego__x_o" in self._necessary_predicates:
                    predicate_trace["keeps_safe_distance_prec__x_ego__x_o"][
                        other_vehicle.id
                    ][time_step] = self.keeps_safe_distance_prec(
                        time_step, ego_vehicle, other_vehicle, operating_mode
                    )
                if "keeps_safe_distance_prec__x_o__x_ego" in self._necessary_predicates:
                    predicate_trace["keeps_safe_distance_prec__x_o__x_ego"][
