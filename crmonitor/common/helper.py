...
from typing import Dict, Union, List, Tuple, Iterable, Sequence, Callable
...
def get_curvilinear_coordinate_system(ref_path: np.ndarray) -> Callable[[np.ndarray], np.ndarray]:
    """
    ...
    def fn(cartesian_points: np.ndarray) -> np.ndarray:
        ...
        s = (cartesian_points_seg_local[..., 0] + m_b * cartesian_points_seg_local[..., 1]) / (seg_length - cartesian_points_seg_local[..., 1] * (m_t - m_b))
        # Dimensions (cart pts, segs, xy)
