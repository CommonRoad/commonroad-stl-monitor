...
class CommonRoadObstacleEvaluation:
    ...
    def __init__(self, config_path: str, backend: Backend = Backend.PythonMTL):
        ...
        self._ego_vehicle_param = create_ego_vehicle_param(
            config.get("ego_vehicle_param"), self._simulation_param
        )
        ...
        self._activated_traffic_rule_sets = traffic_rules.get(
    ...
    def _execute_evaluation(
        ...
        self._road_network = RoadNetwork(
            scenario.lanelet_network, self._road_network_param
        )
        dispatcher = TrafficRuleDispatcher(
            self._traffic_rules_forward,
            self._traffic_rules_backward,
        ...
        for ego in scenario.dynamic_obstacles:
            ...
            vehicle_evaluation.append(
                (
                    ego_vehicle.id,
                    dispatcher.evaluate_trajectory(ego_vehicle, other_vehicles),
                )
            )
    ...
    def evaluate_scenario(
        ...
        try:
            ...
            print(
                ...
                " could not be evaluated: Runtime Error",
            )
            traceback.print_exc()
            return
            ...
            print(
                "scenario ",
                scenario.benchmark_id,
                " could not be evaluated: Attribute Error",
            )
            traceback.print_exc()
            return
        except KeyError:
            print(
                "scenario ", scenario.benchmark_id, " could not be evaluated: Key Error"
            )
            traceback.print_exc()
            return
        except ValueError:
            print(
                "scenario ",
                scenario.benchmark_id,
                " could not be evaluated: Value Error",
            )
            traceback.print_exc()
            return
        except IndexError:
            print(
                "scenario ",
                scenario.benchmark_id,
                " could not be evaluated: Index Error",
            )
            traceback.print_exc()
            return
    ...
    def _init_eval_dict(self) -> Tuple[Dict[str, int], Dict[str, bool]]:
        ...
        for traffic_rule_set_id in self._activated_traffic_rule_sets:
            for rule_name in self._traffic_rule_sets.get(traffic_rule_set_id):
                ...
                elif eval_dict.get(rule_name) is None and "_".join(
                    rule_name.split("_", 2)[:2]
                ):
    ...
    def evaluate_result(self, result, scenario_name):
        ...
        for vehicle in result:
            out_string = (
                "scenario: " + scenario_name + " - evaluated obs-id: " + str(vehicle[0])
            )
