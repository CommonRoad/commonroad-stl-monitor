...
CurvilinearCoordinateSystem::CurvilinearCoordinateSystem(EigenPolyline reference_path,
  ...
  if (reference_path.size() < 3) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem> Reference path must have at least 3 points.");
  }
...
EigenPolyline CurvilinearCoordinateSystem::projectionDomainBorder() const {
  ...
  for (auto it = boost::begin(boost::geometry::exterior_ring(this->projection_domain_));
       it != boost::end(boost::geometry::exterior_ring(this->projection_domain_)); ++it) {
...
EigenPolyline CurvilinearCoordinateSystem::curvilinearProjectionDomainBorder() const {
  ...
  for(auto it = boost::begin(boost::geometry::exterior_ring(this->curvilinear_projection_domain_));
      it != boost::end(boost::geometry::exterior_ring(this->curvilinear_projection_domain_)); ++it) {
    double x = boost::geometry::get<0>(*it);
    double y = boost::geometry::get<1>(*it);
    border.push_back(Eigen::Vector2d(x, y));
  }
...
std::vector<double> CurvilinearCoordinateSystem::segmentsLongitudinalCoordinates() const {
...
void CurvilinearCoordinateSystem::setCurvature(std::vector<double> curvature) {
  if(curvature.size() != this->segment_longitudinal_coord_.size()) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/setCurvature> Curvature values must be given for "
                                "each segment.");
  }
...
std::tuple<double, double> CurvilinearCoordinateSystem::curvatureRange(double s_min, double s_max) const {
  ...
  if (it_min == this->curvature_.begin()) {
    ...
    throw std::invalid_argument("<CurvilinearCoordinateSystem/curvatureRange> Longitudinal position is outside of "
                                "coordinate system.");
  } else {
    it_min--;
  }
...
double CurvilinearCoordinateSystem::maximumCurvatureRadius() const {
  if (std::isnan(this->max_curvature_radius_)) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/getMaximumCurvatureRadius> Maximum curvature radius "
                                "must be set first.");
  }
...
double CurvilinearCoordinateSystem::minimumCurvatureRadius() const {
  if (std::isnan(this->min_curvature_radius_)) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/getMinimumCurvatureRadius> Minimum curvature radius "
                                "must be set first.");
  }
...
double CurvilinearCoordinateSystem::maximumCurvature() const {
  if (std::isnan(this->max_curvature_)) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/getMaximumCurvature> Maximum curvature "
                                "must be set first.");
  }
...
double CurvilinearCoordinateSystem::minimumCurvature() const {
  if (std::isnan(this->min_curvature_)) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/getMinimumCurvature> Minimum curvature "
                                "must be set first.");
  }
...
Eigen::Vector2d CurvilinearCoordinateSystem::normal(double s) const {
  ...
  if (idx < 0) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/normal> Longitudinal coordinate "
...
Eigen::Vector2d CurvilinearCoordinateSystem::tangent(double s) const {
  auto idx = this->findSegmentIndex(s);
  if (idx < 0) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/tangent> Longitudinal coordinate "
                                "outside of projection domain.");
  }
...
Eigen::Vector2d CurvilinearCoordinateSystem::convertToCartesianCoords(double s, double l) const {
  bool is_in_projection_domain = this->curvilinearPointInProjectionDomain(s, l);
  if (!is_in_projection_domain) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/convertToCurvilinearCoords> "
                                "Coordinate outside of projection domain.");
  }
  int idx = this->findSegmentIndex(s);
  return this->segment_list_[idx]->convertToCartesianCoords(s - this->segment_longitudinal_coord_[idx], l);
}

Eigen::Vector2d CurvilinearCoordinateSystem::convertToCurvilinearCoords(double x, double y) const {
  int segment_idx = -1;
  return this->convertToCurvilinearCoordsAndGetSegmentIdx(x, y, segment_idx);
}

Eigen::Vector2d CurvilinearCoordinateSystem::convertToCurvilinearCoordsAndGetSegmentIdx(
  ...
  if (!is_in_projection_domain) {
    ...
    throw std::invalid_argument("<CurvilinearCoordinateSystem/convertToCurvilinearCoordsAndGetSegmentIdx> Coordinate "
                                "outside of projection domain.");
  }
  ...
  for (int i = 0; i < this->segment_list_.size(); i++) {
    ...
    Eigen::Vector2d curvilinear_point = segment->convertToCurvilinearCoords(x, y, lambda);
    if (std::isgreaterequal(lambda + 10e-8, 0.0) && std::islessequal(lambda - 10e-8, 1.0)) {
      candidates.push_back(std::pair<Eigen::Vector2d, int>(curvilinear_point, i));
    }
  }
  ...
  return candidates[0].first + Eigen::Vector2d(this->segment_longitudinal_coord_[segment_idx], 0.);
}

EigenPolyline CurvilinearCoordinateSystem::convertListOfPointsToCurvilinearCoords(
    const EigenPolyline &points,
  ...
  std::vector < std::vector < std::tuple < int, double, double >> > transformed_coordinates_and_segment_idx;
  transformed_coordinates_and_segment_idx = this->convertToCurvilinearCoords(points_in, num_omp_threads);
  ...
  if(transformed_coordinates_and_segment_idx.size() == 1) {
    for (const auto p : transformed_coordinates_and_segment_idx[0]) {
      transformed_points.push_back(Eigen::Vector2d(std::get<1>(p), std::get<2>(p)));
    }
  }
...
EigenPolyline CurvilinearCoordinateSystem::convertRectangleToCartesianCoords(
    double s_lo, double s_hi, double l_lo, double l_hi, std::vector<EigenPolyline>& triangle_mesh) const {
  int idx_lo = this->findSegmentIndex(s_lo);
  int idx_hi = this->findSegmentIndex(s_hi);
  if (idx_lo < 0 || idx_hi < 0) {
    throw std::invalid_argument("<CurvilinearCoordinateSystem/convertRectangleToCurvilinearCoords> "
                                "Coordinate outside of projection domain.");
  }
  ...
  for (int idx = idx_lo; idx <= idx_hi; idx++) {
    ...
    double segment_s_lo = std::max(0.0, s_lo - this->segment_longitudinal_coord_[idx]);
    double segment_s_hi = std::min(segment->length_, s_hi - this->segment_longitudinal_coord_[idx]);
    auto tmp = segment->convertRectangleToCartesianCoords(segment_s_lo, segment_s_hi, l_lo, l_hi);
...
void CurvilinearCoordinateSystem::convertPolygonToCurvilinearCoords(
  ...
  this->convertListOfPolygonsToCurvilinearCoordsAndRasterize(
      polygon_in, groups, 1, 4, transformed_polygons, transformed_polygons_rasterized);
...
void CurvilinearCoordinateSystem::convertListOfPolygonsToCurvilinearCoordsAndRasterize(
  ...
  this->determineSubsetsOfMultiPolygonsWithinProjectionDomain(
      polygons, groups_of_polygons, num_omp_threads, clipped_polygon_all, clipped_polygon_groups_all);
...
#pragma omp for nowait
    for (std::vector<EigenPolyline>::const_iterator polygon_it =
        clipped_polygon_all.begin(); polygon_it < clipped_polygon_all.end(); ++polygon_it) {
      ...
      bool success = this->addPointsAtSegmentTransition(
          *polygon_it, poly_curvil_coordinates[cur_poly_index], transformed_polygon, indices);
      ...
      if (success && transformed_polygon_thread.size() > 0) {
        this->rasterizeListOfTransformedPolygonsInProjectionDomain(
            transformed_polygon_thread,
            segment_indices_thread,
...
bool CurvilinearCoordinateSystem::addPointsAtSegmentTransition(
    const EigenPolyline &polygon,
    const std::vector<std::tuple<int, double, double>> &curvilinear_coordinates_and_segment_idx,
    EigenPolyline &curvilinear_polygon,
    std::set<int> &segment_indices) const {
  ...
  previous_segment_idx = std::get<0>(curvilinear_coordinates_and_segment_idx[0]);
  curvilinear_point = Eigen::Vector2d(std::get<1>(curvilinear_coordinates_and_segment_idx[0]),
  ...
  for (int i = 1; i < polygon_vertices.size(); i++) {
    ...
    curvilinear_point = Eigen::Vector2d(std::get<1>(curvilinear_coordinates_and_segment_idx[i]),
                                        std::get<2>(curvilinear_coordinates_and_segment_idx[i]));
    segment_indices.insert(segment_idx);
    // check if new point lies in a different segment than the previous point, if yes, compute intermediate
    // points for each segment in between
    if (segment_idx == previous_segment_idx) {
      curvilinear_polygon.push_back(curvilinear_point);
    } else {
      std::list<int> indices_range = this->determineIndicesRange(segment_idx, previous_segment_idx);
      segment_indices.insert(indices_range.begin(), indices_range.end());
      this->interpolatePointsBetweenSegments(
          polygon_vertices[i - 1], polygon_vertices[i], indices_range, curvilinear_polygon);
...
std::list<int> CurvilinearCoordinateSystem::determineIndicesRange(int segment_idx,
...
void CurvilinearCoordinateSystem::interpolatePointsBetweenSegments(const Eigen::Vector2d cartesian_point,
  ...
  for (const auto &k : indices_range) {
    ...
    bool intersect = geometry::util::intersectionSegmentSegment(
        segment->pt_2() - (this->default_projection_domain_limit_ + 10.0) * segment->normalSegmentEnd(),
        segment->pt_2() + (this->default_projection_domain_limit_ + 10.0) * segment->normalSegmentEnd(),
        next_cartesian_point, tmp_vertices.back(),
        intersection_point);
    ...
    if (intersect) {
      ...
      curvilinear_point += Eigen::Vector2d(this->segment_longitudinal_coord_[k], 0);

      // in the case that the last transformed point lies near the new segment, numerically errors may occur
      // when constructing a polygon, since the new curvilinear_point and the last curvilinear point are
      // almost the same. We skip that point and do not add it to the coordinate list
      if (k == indices_range.front() && first_curvilinear_point.isApprox(curvilinear_point, 10e-8)) {
...
void CurvilinearCoordinateSystem::rasterizeListOfTransformedPolygonsInProjectionDomain(
  ...
  for (const auto &polygon : transformed_polygons) {
    ...
    this->rasterizeTransformedPolygonInProjectionDomain(polygon, segment_indices, rectangle_list);
    transformed_polygons_rasterized.insert(transformed_polygons_rasterized.end(),
                                           rectangle_list.begin(), rectangle_list.end());
  }
}
...
void CurvilinearCoordinateSystem::rasterizeTransformedPolygonInProjectionDomain(
  ...
  auto createRectangle = [](EigenPolyline points) {
    ...
    EigenPolyline rectangle{Eigen::Vector2d(x_min, y_min), Eigen::Vector2d(x_max, y_min),
                            Eigen::Vector2d(x_max, y_max), Eigen::Vector2d(x_min, y_max)};
    return rectangle;
  };

  std::set<int> segment_indices_extended(segment_indices.begin(), segment_indices.end());
  segment_indices_extended.insert(*segment_indices.rbegin() + 1);
  if (*segment_indices.begin() > 0) { segment_indices_extended.insert(*segment_indices.begin() - 1); }

  double last_maximum_s_coord = std::numeric_limits<double>::infinity();
  std::set<int>::iterator it;
  for (it = std::next(segment_indices_extended.begin()); it != segment_indices_extended.end(); ++it) {
    double s_min = std::min(this->segment_longitudinal_coord_[*std::prev(it)],
                            last_maximum_s_coord) - 10e-3;
    double s_max = this->segment_longitudinal_coord_[*it] + 10e-3;

    EigenPolyline points_in_segment;
    if ((it != segment_indices_extended.end()) && (std::next(it) == segment_indices_extended.end())) {
      copy_if(transformed_polygon.begin(), transformed_polygon.end(), back_inserter(points_in_segment),
              [s_min](Eigen::Vector2d p) {
                return std::isgreaterequal(p[0], s_min);
              });
    } else {
      copy_if(transformed_polygon.begin(), transformed_polygon.end(), back_inserter(points_in_segment),
              [s_min, s_max](Eigen::Vector2d p) {
                return (std::isgreaterequal(p[0], s_min) && std::islessequal(p[0], s_max));
              });
    }
...
bool CurvilinearCoordinateSystem::curvilinearPointInProjectionDomain(double s, double l) const {
  return boost::geometry::within(point_type(s, l), this->curvilinear_projection_domain_);
}
...
std::vector<EigenPolyline> CurvilinearCoordinateSystem::determineSubsetOfPolygonWithinCurvilinearProjectionDomain(
    const EigenPolyline &polygon) const {
  return this->polygonWithinProjectionDomain(polygon, this->curvilinear_projection_domain_);
}

void CurvilinearCoordinateSystem::determineSubsetsOfMultiPolygonsWithinProjectionDomain(
    const std::vector<EigenPolyline> &polygons,
    const std::vector<int> groups_of_polygons,
...
#pragma omp for nowait
    for (std::vector<EigenPolyline>::const_iterator polygon_it = polygons.begin(); polygon_it < polygons.end();
         ++polygon_it) {
      std::vector<EigenPolyline> polygon_in_proj_domain = this->determineSubsetOfPolygonWithinProjectionDomain(
          *polygon_it);
    ...
    groups_of_polygons_in_projection_domain.insert(groups_of_polygons_in_projection_domain.end(),
...
std::vector<EigenPolyline> CurvilinearCoordinateSystem::polygonWithinProjectionDomain(
  ...
  if (boost::geometry::within(poly_aabb, projection_domain)) {
    ...
    boost::geometry::intersection(poly_in, projection_domain, parts_in_projection_domain);
...
void CurvilinearCoordinateSystem::createSegment(Eigen::Vector2d pt_1,
                                                ...
                                                Eigen::Vector2d t_1,
                                                Eigen::Vector2d t_2) {
  this->segment_list_.push_back(std::make_unique<Segment>(pt_1, pt_2, t_1, t_2));
...
std::vector<int> CurvilinearCoordinateSystem::findCandidatePointsInSegment(
  ...
  auto low = std::lower_bound(pair_projected_coord_and_id.begin(),
                              pair_projected_coord_and_id.end(),
                              val_min,
                              [](const std::pair<double, int> &lhs, const double rhs) -> bool {
                                return lhs.first < rhs;
                              });

  auto high = std::lower_bound(pair_projected_coord_and_id.begin(),
                               pair_projected_coord_and_id.end(),
                               val_max,
                               [](std::pair<double, int> lhs, const double rhs) -> bool {
                                 return lhs.first < rhs;
                               });
  std::vector<int> candidates_indices;
  for (auto it = low; it < high; it++) {
    int point_id = pair_projected_coord_and_id[it - pair_projected_coord_and_id.begin()].second;
...
void CurvilinearCoordinateSystem::determineCurvilinearCoordinatesAndSegmentIdx(
    const std::vector < std::vector < std::tuple < int, int >> >& candidate_segments_of_points,
    const std::vector<Eigen::RowVectorXd, Eigen::aligned_allocator<Eigen::RowVectorXd>>& s_coord,
    const std::vector<Eigen::RowVectorXd, Eigen::aligned_allocator<Eigen::RowVectorXd>>& l_coord,
    int num_omp_threads,
    std::vector < std::vector < std::tuple < int, double, double >> >& groups_of_curvil_points) const {
  // settings for OMP
  omp_set_dynamic(0);
  omp_set_num_threads(num_omp_threads);
  ...
  #pragma omp for nowait
    for (int i = 0; i < groups_of_curvil_points.size(); i++) {
      for (int j = 0; j < groups_of_curvil_points[i].size(); j++) {
        ...
        int best_segment = -1;
        int best_idx = -1;
        // there exist more than one valid segment for the point -> take the one with minimal lateral coordinate
        if (candidate_segments_of_points[orig_idx].size() > 1) {
          ...
          throw std::logic_error("<CurvilinearCoordinateSystem/convertToCurvilinearCoords> "
                                 "Coordinate outside of projection domain.");
        }
        groups_of_curvil_points[i][j] = std::make_tuple(
            best_segment,
            s_coord[best_segment][best_idx],
...
void CurvilinearCoordinateSystem::approximateProjectionDomain(double eps) {
  ...
  EigenPolyline projection_domain_border = this->computeProjectionDomainBorder(min_radius, max_radius);
  ...
  for (EigenPolyline::iterator it = projection_domain_border.begin();
    ...
    boost::geometry::append(this->projection_domain_, point_type((*it)[0], (*it)[1]));
  }
}
...
void CurvilinearCoordinateSystem::approximateCurvilinearProjectionDomain() {
  ...
  this->convertPolygonToCurvilinearCoords(this->projectionDomainBorder(), curvilinear_projection_domain);

  if (curvilinear_projection_domain.size() != 1) {
    std::cout << curvilinear_projection_domain.size() << std::endl;
    throw std::logic_error("<CurvilinearCoordinateSystem/approximateCurvilinearProjectionDomain> could not"
                           " generate curvilinear projection domain.");
  }

  for (EigenPolyline::reverse_iterator it = curvilinear_projection_domain[0].rbegin();
       it != curvilinear_projection_domain[0].rend(); it++) {
    boost::geometry::append(this->curvilinear_projection_domain_, point_type((*it)[0], (*it)[1]));
  }

  if (!curvilinear_projection_domain[0].front().isApprox(curvilinear_projection_domain[0].back())) {
...
std::tuple<double, double> CurvilinearCoordinateSystem::computeProjectionDomainLimits(double eps) const {
  ...
  for (int i = 0; i < this->segment_list_.size() - 1; i++) {
    ...
    Eigen::Vector2d p_1 = segment_1->pt_2() - 50.0 * segment_1->normalSegmentEnd();
    Eigen::Vector2d p_2 = segment_1->pt_2() + 50.0 * segment_1->normalSegmentEnd();

    const auto &segment_2 = this->segment_list_[i + 1];
    Eigen::Vector2d p_3 = segment_2->pt_2() - 50.0 * segment_2->normalSegmentEnd();
    Eigen::Vector2d p_4 = segment_2->pt_2() + 50.0 * segment_2->normalSegmentEnd();

    Eigen::Vector2d intersection_point(0., 0.);
    bool intersect = geometry::util::intersectionLineLine(p_1, p_2, p_3, p_4, intersection_point);
    if (intersect) {
      Eigen::Vector2d d_1 = intersection_point - segment_1->pt_2();
      Eigen::Vector2d d_2 = intersection_point - segment_2->pt_2();
      double dot_d_1 = (segment_1->normalSegmentEnd()).dot(intersection_point - segment_1->pt_2());
...
EigenPolyline CurvilinearCoordinateSystem::computeProjectionDomainBorder(double min_radius, double max_radius) {
  ...
  if (!this->upper_projection_domain_border_.front().isApprox(this->lower_projection_domain_border_.front())) {
    projection_domain_border.push_back(this->upper_projection_domain_border_.front());
  }
...
void CurvilinearCoordinateSystem::computeBestProjectionAxisForSegments() {
  for (int i = 0; i < this->segment_list_.size(); i++) {
    auto &segment = this->segment_list_[i];
    // find intersection points of segment normals n_1_ and n_2_ with the upper and lower projection domain border
    EigenPolyline pt(4);
    if (i == 0 || i == 1 || i == this->segment_list_.size() - 1) {
      pt[0] = this->upper_projection_domain_border_[this->upper_projection_domain_border_.size() - 1];
      pt[1] = this->upper_projection_domain_border_[0]; // end of point 1
      pt[2] = this->lower_projection_domain_border_[this->lower_projection_domain_border_.size() - 1];
    ...
    // find minimum and maximum longitudinal and lateral coordinates when projecting current segment onto the
    // longitudinal and lateral axis (axis-aligned bounding box of current segment)
    ...
    // find minimum and maximum longitudinal and lateral coordinates when projecting current segment onto two diagonal
    // axes (45 degrees) (find orientated bounding box of current segment)
    ...
    for (auto p: pt) {
      ...
      if (proj_p.x() < min_segm_x)
        min_segm_x = proj_p.x();
      if (proj_p.x() > max_segm_x)
        max_segm_x = proj_p.x();

      if (proj_p.y() < min_segm_y)
        min_segm_y = proj_p.y();
      if (proj_p.y() > max_segm_y)
        max_segm_y = proj_p.y();
    }
    ...
    // find axes where the distance to the road boundary within current segment is minimal
    ...
    int best_segm_axis_id = std::min_element(diff_vec.begin(), diff_vec.end()) - diff_vec.begin();
...
CurvilinearCoordinateSystem::convertToCurvilinearCoords(const std::vector<EigenPolyline> &groups_of_points,
                                                        int num_omp_threads) const {
...
#pragma omp for nowait
  ...
  std::vector<Eigen::RowVectorXd, Eigen::aligned_allocator<Eigen::RowVectorXd>> s_results(this->segment_list_.size());
  std::vector<Eigen::RowVectorXd, Eigen::aligned_allocator<Eigen::RowVectorXd>> l_results(this->segment_list_.size());

#pragma omp parallel
  {
#pragma omp for nowait
    for (int segment_index = 0; segment_index < this->segment_list_.size(); segment_index++) {
      // find points which are potentially within the current segment
      std::vector<int> candidates_indices;
      if (this->best_segm_axis_[segment_index] == ProjectionAxis::X_AXIS) {
        candidates_indices = this->findCandidatePointsInSegment(segment_index, pairs_in_x);
      } else if (this->best_segm_axis_[segment_index] == ProjectionAxis::X_AXIS_ROTATED) {
        candidates_indices = this->findCandidatePointsInSegment(segment_index, pairs_in_x_rotated);
      } else if (this->best_segm_axis_[segment_index] == ProjectionAxis::Y_AXIS) {
        candidates_indices = this->findCandidatePointsInSegment(segment_index, pairs_in_y);
      } else if (this->best_segm_axis_[segment_index] == ProjectionAxis::Y_AXIS_ROTATED) {
        candidates_indices = this->findCandidatePointsInSegment(segment_index, pairs_in_y_rotated);
      }
      // get the coordinates of points which might be in the current segment
      Eigen::Matrix2Xd candidate_points_in_segment(2, candidates_indices.size());
      int offset_cols = 0;
      for (auto el: candidates_indices) {
        candidate_points_in_segment.middleCols(offset_cols++, 1) = points_in.middleCols(el, 1);
      }
      ...
      Eigen::Matrix2Xd valid_candidate_points_in_segment(2, candidates_indices.size());
      int k = 0;
      int n_cand = 0;
      for (auto orig_ind : candidates_indices) {
        if (dividers(k) > 0.) {
          if ((scaled_lambdas(k) + 10e-8 >= 0.0) && (scaled_lambdas(k) - 10e-8 <= dividers(k))) {
            valid_lambdas(n_cand) = scaled_lambdas(k) / dividers(k);
            valid_candidates_y(n_cand) = candidate_points_in_segment_local(1, k);
            omp_set_lock(&writelock);
            candidate_segments_of_points[orig_ind].push_back(std::make_tuple(segment_index, n_cand));
            omp_unset_lock(&writelock);
            valid_candidate_points_in_segment.middleCols(n_cand, 1) = candidate_points_in_segment.middleCols(k, 1);
            n_cand++;
          }
        }
        k++;
      ...
      if (n_cand) {
        Eigen::Matrix2Xd candidates_point_in = valid_candidate_points_in_segment.block(0, 0, 2, n_cand);
        Eigen::RowVectorXd lambdas = valid_lambdas.block(0, 0, 1, n_cand);
        Eigen::RowVectorXd p_local_y = valid_candidates_y.block(0, 0, 1, n_cand);
        // compute signed pseudo distance
        Eigen::RowVectorXd pseudo_distance = segment->computePseudoDistance(lambdas, candidates_point_in);
        // compute signed pseudo distance
        ...
        Eigen::RowVectorXd lengths = Eigen::RowVectorXd::Constant(n_cand, segment->length_);
  ...
  std::vector < std::vector < std::tuple < int, double, double >> > groups_of_curvil_points(groups_of_points.size());
  ...
  this->determineCurvilinearCoordinatesAndSegmentIdx(
      candidate_segments_of_points, s_results, l_results, num_omp_threads, groups_of_curvil_points);
  omp_destroy_lock(&writelock);
