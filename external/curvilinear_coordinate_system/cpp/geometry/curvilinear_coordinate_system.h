...
enum class ProjectionAxis { X_AXIS = 0, Y_AXIS = 1, X_AXIS_ROTATED = 2, Y_AXIS_ROTATED = 3 };
 ...
 */
  ...
  /**
   * Transforms a Cartesian point to the curvilinear frame and returns the segment index, in which the
   * point is contained.
   *
  ...
  /**
   * Transforms a rectangle in the curvilinear coordinates to the Cartesian coordinates. Additionally, a triangle mesh
   * of the resulting polygon is generated.
   *
  ...
  /**
   * Transforms different multipolygons which are inside the unique projection domain of the
   * curvilinear coordinate system.
   * Furthermore, the transformed multipolygons are rasterized meaning that the polygons are over-approximated with
  ...
  /**
   * Validates if a point in global coordinates is within the unique projection domain of the curvilinear coordinate
   * system.
   *
   * @param x x-coordinate in the Cartesian coordinate system
   * @param y y-coordinate in the Cartesian coordinate system
   * @return true, if the point is inside or on the boundary of the projection domain,
            false, if the point is outside of the boundary of the projection domain.
   */
  ...
  /**
   * Computes the parts of a polygon (given in curvilinear coordinates) which are inside the curvilinear projection
   ...
   */
  std::vector<EigenPolyline> determineSubsetOfPolygonWithinCurvilinearProjectionDomain(
      const EigenPolyline &polygon) const;

  /**
   * Computes the parts of different multipolygons which are inside the unique projection domain of the
   * curvilinear coordinate system.
   *
   * @param[in] polygons list of polygons
   * @param[in] groups_of_polygons list of IDs indicating the group to which a polygon belongs
   ...
   * @param[out] groups_of_polygons_in_projection_domain indices indicating to which group the clipped polygon belongs
   */
 ...
 private:
   ...
   *
   * @param groups_of_points vector of point clouds in the global coordinate frame
   * @param num_omp_threads number of OMP threads for computation
   * @return groups of transformed points and corresponding segment index, in which the points are contained
   */
  std::vector<std::vector<std::tuple < int, double, double>>> convertToCurvilinearCoords(
      const std::vector<EigenPolyline> &groups_of_points,
      int num_omp_threads) const;

  /**
   * Computes the parts of a polygon which are inside the unique projection domain of the curvilinear coordinate system.
   * The projection domain can be either the curvilinear or global projection domain.
   *
   * @param polygon vertices of the boundary of the polygon; vertices must be sorted clockwise and given as closed list
   *        (the last vertex must be the same as the first one)
   * @param projection_domain curvilinear or global projection domain of coordinate system
   * @return parts of the polygon which are inside the projection domain.
   */
   ...
   */
  void createSegment(Eigen::Vector2d pt_1, Eigen::Vector2d pt_2, Eigen::Vector2d t_1, Eigen::Vector2d t_2);

  /**
   * Approximates the unique projection domain of the coordinate system.
   *
   * @param eps parameter to shrink the projection domain which avoids numerical errors
   */
  ...
  /**
   * Computes the maximum positive and negative distance from the reference path, for which points can be transformed
   * uniquely.
   *
   * @param eps parameter to shrink the projection domain which avoids numerical errors
  ...
  /**
   * Computes the border of the unique projection domain. The points are sorted clockwise and the
   * first point coincides with the last point.
   *
   * @param min_radius maximum distance of border to the right of the reference path
   * @param max_radius minimum distance of border to the left of the reference path
   * @return border points of projection domain.
   */
  EigenPolyline computeProjectionDomainBorder(double min_radius, double max_radius);

  /**
   * Computes a projection axis for each segment to quickly determine global points that lie within the segment.
   * Only the points that lie between the minimum and the maximum coordinates projected on the axis
   * are considered to be within the segment.
   *
   * First, the intersections between the normals n_1_ and n_2_ of the segment and the projection domain border
   * are found, resulting in 4 intersection points.
   * Second, the starting point pt_1_ , the end point pt_2_, as well as the found intersection points
   * of the segment are projected onto the x- and y-axes of the global coordinate system.
   * All candidate points for the projection onto the segment must lie within the bounding box
   * (x_min; x_max; y_min; y_max).
   * In addition, the six points are projected onto the two diagonal axes that are obtained through rotating the
   * x and y axes counterclockwise by 45 degrees.
   * The best axis for each segment is the axis for which the difference between the maximum and the minimum
   * projection coordinate is minimal.
   *
   */
   ...
   * @param curvilinear_coordinates_and_segment_idx transformed boundary of polygons and corresponding segment indices
  ...
  /**
   * Given two Cartesian points (cartesian_point and next_cartesian_point), the function computes the intersection
   * points of the line connecting the two points and the segment normals.
   *
   * @param[in] cartesian_point
   * @param[in] next_cartesian_point
   * @param[in] indices_range IDs of segments whose normals intersect with the line connecting cartesian_point
   *                      and next_cartesian_point
   * @param[in/out] transformed_polygon transformed intersection points are appended to transformed_polygon
   */
  void interpolatePointsBetweenSegments(const Eigen::Vector2d cartesian_point,
   ...
   * @param segment_indices IDs of all segments containing all transformed polygons
   ...
   * @param segment_indices  IDs of all segments containing the transformed polygon
  ...
  /**
   * Given a list of coordinates projected to the corresponding projection axis of a segment, the function returns
   * the indices of the points which may be inside the segment.
   *
   * @param segment_idx ID of the segment
   * @param pair_projected_coord_and_id projected coordinates to the projection axis of the segment with ID segment_idx
   *                                    + unique ID for each projected coordinate
   * @return IDs of projected coordinates which are candidate that lie inside the segment with ID segment_idx
   */
  std::vector<int> findCandidatePointsInSegment(
      int segment_idx, const std::vector<std::pair<double, int>> &pair_projected_coord_and_id) const;

  /**
   * Given a set of candidate segments for each point, the transformed longitudinal and lateral coordinates for each
   * candidate segment, the function computes the curvilinear coordinates.
   */
  ...
  /**
   * Finds the corresponding segment for a given longitudinal coordinate of a point.
   *
   * @param s longitudinal coordinate
