...
class TestPositionPredicates(unittest.TestCase):
    ...
    def testOnIncomingLeftOf(self):
        scenario, _ = CommonRoadFileReader(str("../scenarios/test_intersection/DEU_test_traffic_signs_1.xml")).open(
                lanelet_assignment=True)
        world = World.create_from_scenario(scenario)
        road_network = RoadNetwork(scenario.lanelet_network, self.config.get("road_network_param"))
        ego_vehicle = world.vehicle_by_id(1002)
        target_vehicle = world.vehicle_by_id(1001)
