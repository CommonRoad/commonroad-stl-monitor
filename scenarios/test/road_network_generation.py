...
def create_access_ramp_end(x_start:int, l_id: int, lanelet_length: int, number_lanelets_lane: int):
    ...
    point_list = []
    for idx in range(10):
        point = np.array([x_start + idx, y_right[0]])
        point_list.append(point)
    for idx in range(len(x_right)):
        point = np.array([x_right[idx], y_right[idx]])
        point_list.append(point)
    ...
    point_list = []
    for idx in range(10):
        point = np.array([x_start + idx , y_left[0]])
        point_list.append(point)
    for idx in range(len(x_right)):
        point = np.array([x_left[idx], y_left[idx]])
        point_list.append(point)
    ...
    point_list = []
    for idx in range(10):
        point = np.array([x_start + idx, y_center[0]])
        point_list.append(point)
    for idx in range(len(x_right)):
        point = np.array([x_center[idx], y_center[idx]])
        point_list.append(point)
...
def create_access_ramp(road_length: int, num_lanelets_per_lane: int, lanelet_length: int):
    ...
    for lanelet_idx in range(l_id_start+1, l_id_start+num_lanelets_per_lane-2):
        ...
        if lanelet_idx % num_lanelets_per_lane != 0.0:
            ...
            successor = None
    lanelet_end = create_access_ramp_end(road_length-2*lanelet_length, l_id_start+num_lanelets_per_lane-2,
