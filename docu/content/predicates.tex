...
\begin{align*}
\begin{split}
&\unnecessaryBraking(\egoState, \aEgo) \iff \\
&\qquad \aEgo < 0 \implies \big(\nexists \text{o} \in \{1 ... N\} : \inFrontOfEgoVehiclePredicate(\kState, \otherState)  \\
&\qquad \quad \land \aEgo < \aAbrupt \big) \lor \big(\exists \text{o} \in \{1 ... N\} : \\
&\qquad \quad \inFrontOfEgoVehiclePredicate(\kState, \otherState)  \land (\aEgo - \aOther) <  \aAbrupt \big),
\end{split}
\end{align*}
We assume this information is provided by another system.
\todo{Allow violation of rule if safe distance is violated.}
