...
Concretization &  A vehicle shall not exceed the allowed speed limit of the lanes it drives on and shall \\
& not exceed the speed limit which is necessary to ensure collision freedom with \\
& respect to vehicles  outside the field of view.\\

Predicates & $\laneSpeedLimitPredicate$, $\fovSpeedLimitPredicate$\\
...
& by traffic signs to lanes is given. If no speed limit is given only, \\ 
& the speed limit caused by the field of view must be considered (traffic sign based \\ 
& speed limit set to infinity). The velocity with respect to vehicles inside the field of view \\
& is considered in the safe distance traffic rule concretization.\\

LTL & G$(\laneSpeedLimitPredicate \land \fovSpeedLimitPredicate)$\\
MTL & G$(\laneSpeedLimitPredicate \land \fovSpeedLimitPredicate)$\\
...
\begin{table}[H]
	...
	\begin{tabular}[t]{ll}
		...
		STL & G($v_\mathrm{o}^\mathrm{min} - \vego \leq v_\mathrm{min}^\mathrm{slow})$\\
		\bottomrule
	\end{tabular}
\end{table}
